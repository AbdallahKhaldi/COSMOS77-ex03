"""JSON + .env config loader for COSMOS77-ex03 (CLAUDE.md rule 4).

Every module reads its tunables through :class:`Config`, so topic, language,
model, provider, chapter count, and paths are never hardcoded. ``setup.json`` and
``providers.json`` are version-checked at load; ``.env`` supplies the secret
``GEMINI_API_KEY``. A future migration to YAML/pydantic touches only this file.
"""
...
from cosmos77_ex03.shared.version import validate_config_version
...
class Config:
    ...
    def get(self, dot_path: str, default: Any = _SENTINEL) -> Any:
        """Return the value at ``dot_path`` (e.g. ``crew.num_chapters``).
    ...
    def article(self) -> dict[str, Any]:
        """Return the ``article`` section (title, language, target_pages, author...)."""
        return dict(self.get("article", default={}))

    def crew(self) -> dict[str, Any]:
        """Return the ``crew`` section (process, parallel_writers, num_chapters, max_rpm)."""
        return dict(self.get("crew", default={}))

    def paths(self) -> dict[str, str]:
        """Return the ``paths`` section (output_dir, tex_dir, figures_dir)."""
